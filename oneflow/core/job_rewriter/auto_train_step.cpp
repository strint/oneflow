...
Maybe<void> AutoTrainStep::Apply(Job* job, JobPassCtx* ctx) const {
  ...
  // note(strint): 创建一存储step_index的variable
  ...
  *variable_conf->mutable_shape()->mutable_dim()->Add() = 1;
  ...
  // note(strint): 创建一个identity op，输入为variable
  ...
  // note(strint): step_index + 1，结果用于后面更新step_index的variable
  ...
  if (train_conf.has_dynamic_loss_scale_policy()) {
    ...
    // note(strint): 利用assign op更新variable
    auto assign_op =
